...
class LDASpace():
    ...
    def token_frequency(self, tokenised_documents):
        ...
        for token in tokenised_documents.features:
            word_freq[token] = 0

        for document in tokenised_documents.documents:
            for token, count in document.words.items():
    ...
    def doc2bow(self, vector):
        return [(self.token_2_id[token], self.token_frequency_map[token]) for token in vector.features]
